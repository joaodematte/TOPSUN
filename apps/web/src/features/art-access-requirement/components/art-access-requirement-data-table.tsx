...
function createProjectsOnArtAccessRequirementColumns(
  thresholds: ProjectStatusThresholds
): ColumnDef<ProjectOnArtAccessRequirement>[] {
  return [
    {
      accessorKey: "projeto",
      cell: ({ getValue }) => {
...
interface ArtAccessRequirementDataTableProps {
  data: ProjectOnArtAccessRequirement[];
  pageSize?: number;
  thresholds: ProjectStatusThresholds;
}

export function ArtAccessRequirementDataTable({
  data,
  pageSize = 15,
  thresholds,
}: ArtAccessRequirementDataTableProps) {
  return (
    <DataTable
      columns={createProjectsOnArtAccessRequirementColumns(thresholds)}
      data={data}
      pageSize={pageSize}
    />
  );
